function write_2pt_data(cd::CData, path::String, name::String, rep::String)
    ...
    # open(path, "w") do io
        ...
        writedlm(io, " ")
        writedlm(io, ["nb kappa 1: $(cd.header.k1)"])
        writedlm(io, ["nb kappa 2: $(cd.header.k2)"])
        writedlm(io, ["nb theta 1: $(cd.header.theta1)"])
        writedlm(io, ["nb theta 2: $(cd.header.theta2)"])
        writedlm(io, " ")

        T = collect(Int64, 0:size(cd.re_data,2)-1)
        for (k,cnfg) in enumerate(cd.vcfg)
            writedlm(io, ["# rep $(rep) conf $(cnfg)"])
            writedlm(io, " ")
            for t in T
                writedlm(io,  [t '\t' cd.re_data[k, t+1] '\t' cd.im_data[k, t+1]])
...
function write_2pt_data(cd::Vector{CData}, path::String, name::String, rep::Vector{String})
    ...
    open(joinpath(path,name), "w") do io
        ...
        writedlm(io, " ")
        writedlm(io, ["nb kappa 1: $(cd[1].header.k1)"])
        writedlm(io, ["nb kappa 2: $(cd[1].header.k2)"])
        writedlm(io, ["nb theta 1: $(cd[1].header.theta1)"])
        writedlm(io, ["nb theta 2: $(cd[1].header.theta2)"])
        writedlm(io, " ")
